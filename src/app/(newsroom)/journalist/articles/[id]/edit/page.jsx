...
export default function EditArticlePage() {
  ...
  const onSubmit = async (data, action) => {
    ...
    const toastId = toast.loading(
      action === "resubmit" ? "Resubmitting..." : "Saving changes..."
    );
    ...
    try {
      ...
      // If resubmitting, call the resubmit API
      if (action === "resubmit") {
        const resubRes = await fetch(`/api/articles/${id}/resubmit`, {
          method: "POST",
        });
        if (!resubRes.ok) {
        ...
        toast.success("Article resubmitted for review!", { id: toastId });
      } else {
        toast.success("Changes saved successfully", { id: toastId });
      }

      router.push(`/journalist/articles/${id}`);
    } catch (error) {
  ...
  return (
    <div className="flex flex-col lg:flex-row h-full min-h-screen bg-slate-50">
      ...
      <div className="flex-1 lg:max-w-4xl border-r border-slate-200 bg-white shadow-sm z-10">
        ...
        {/* Revision Warning */}
        {isRevisionRequested && article?.revision?.comment && (
        ...
        <div className="bg-white rounded-xl border border-slate-200 p-5 shadow-sm space-y-3 mt-auto">
          {isRevisionRequested && (
            <button
              ...
              onClick={handleSubmit((data) => onSubmit(data, "resubmit"))}
              className="w-full flex items-center justify-center gap-2 bg-orange-600 hover:bg-orange-700 text-white font-medium py-2.5 rounded-lg transition-colors shadow-sm disabled:opacity-70"
            >
              <Send size={18} />
              Save & Resubmit
            </button>
