  ...
  const handleLoginFunc = async (data) => {
    ...
    try {
      ...
      if (res) {
        ...
        // Only journalists and admins can access journalist dashboard
        if (role === "journalist" || role === "admin") {
          window.location.href = "/journalist";
        } else {
          setErrorMsg(
            "Access denied. This login is for Journalists only. Your account role is: " +
              (role || "user")
          );
        }
      }
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      await authClient.signIn.social({
        provider: "google",
        // After Google login, server checks role and redirects to journalist or access-denied
        callbackURL: "/api/auth/role-redirect?role=journalist",
      });
    } catch (err) {
      ...
      <div className="w-full max-w-md">
        <div className="bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden">
          ...
          >
            ...
            <p className="text-slate-400 text-sm mt-1">Journalist Portal</p>
          </div>
          ...
          <div className="px-6 sm:px-8 py-6 space-y-4">
            ...
            <form className="space-y-4" onSubmit={handleSubmit(handleLoginFunc)}>
              ...
              >
                {isSubmitting ? "Signing in..." : "Sign In as Journalist"}
              </button>
            </form>
